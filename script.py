...
# Keep model outputs deterministic by removing stale model files.
...
over = SMOTE(sampling_strategy=0.7, random_state=SEED)
...
def train_model(X_train, y_train, plot_name='', class_weight=None):
    ...
    clf = LogisticRegression(C=2, class_weight=class_weight, max_iter=1000, random_state=SEED)
